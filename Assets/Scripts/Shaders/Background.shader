		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				else if (_BlendMode >= 3 && _BlendMode <= 4)
				{
					...
					if (_BlendMode == 3)
					{
						d = abs(sin(_Time.x * _BlendTimeScaling * sqrt((new_x - 0.5)*(new_x - 0.5) + (new_y - 0.5)*(new_y - 0.5)) / sqrt(_BlendWarp)));
					}
					else if (_BlendMode == 4)
					{
						d = fmod(abs(tan(_Time.x * _BlendTimeScaling)) * sqrt((new_x - 0.5)*(new_x - 0.5) + (new_y - 0.5)*(new_y - 0.5)) / sqrt(_BlendWarp) + 1, 1);
					}
